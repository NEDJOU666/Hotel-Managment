...
const Feedback = () => {
  ...
  return (
    <div className="col-span-12  !h-[290px] w-[47%] rounded-xl overflow-y-auto border border-stroke bg-white shadow-gray-300 shadow-md  xl:col-span-6">
        ...
        <div className="relative flex">
          ...
          <div className="absolute right-0 top-full z-40 w-40 space-y-1 rounded-sm border border-stroke bg-white p-1.5 shadow-default  hidden">
            <button className="flex w-full items-center gap-2 rounded-sm px-4 py-1.5 text-left text-sm hover:bg-gray dark:hover:bg-meta-4">
              <svg className="fill-current" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg"><g clip-path="url(#clip0_62_9787)">
                ...
                </path></g><defs><clipPath id="clip0_62_9787"><rect width="16" height="16" fill="white"></rect></clipPath></defs>
              </svg>Edit</button><button className="flex w-full items-center gap-2 rounded-sm px-4 py-1.5 text-left text-sm hover:bg-gray dark:hover:bg-meta-4">
        ...
        <div className="flex flex-col gap-7">
          
            {
              FeedbackData.map((item,index) => {
                return(
                  <>
                  <div key={index} className="relative z-1 flex gap-5.5">
                    <div className="h-16 w-full max-w-16 rounded-full border-[3px] border-stroke dark:border-strokedark flex justify-center items-center"><img alt="User" className="w-[52px] h-[52px] rounded-full" src="https://images.unsplash.com/photo-1600486913747-55e5470d6f40?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1770&q=80" loading="lazy" width="64" height="64" decoding="async" data-nimg="1"/></div><div><p className="text-black dark:text-white"><span className="font-medium">{item.name}</span><span className="text-sm px-1">Commented on Cloud</span><span className="font-medium">{item.whoComment}</span></p><span className=" block text-sm"> 1 hour ago</span><p className="mt-1 text-black text-sm dark:text-white">{item.Descriptiom}</p>
                    </div>
                    <span className="absolute left-8 -z-1 block h-[300%] w-[1px] border-l border-dashed border-stroke dark:border-strokedark">
                    </span>
                  </div>
                  </>
                )
